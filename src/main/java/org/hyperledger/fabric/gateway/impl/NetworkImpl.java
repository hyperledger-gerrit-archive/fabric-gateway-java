...
public final class NetworkImpl implements Network {
    ...
    public Consumer<BlockEvent> addBlockListener(Checkpointer checkpointer, Consumer<BlockEvent> listener) throws GatewayException, IOException {
        synchronized (blockListenerSessions) {
            if (!blockListenerSessions.containsKey(listener)) {
                ListenerSession session = newCheckpointBlockListenerSession(checkpointer, listener);
    ...
    private ListenerSession newCheckpointBlockListenerSession(Checkpointer checkpointer, Consumer<BlockEvent> listener) throws IOException, GatewayException {
        ...
        if (blockNumber == Checkpointer.UNSET_BLOCK_NUMBER) {
            ...
            return new BlockListenerSession(orderedBlockSource, checkpointListener);
        }
        return new ReplayListenerSession(this, checkpointListener, blockNumber);
    }
